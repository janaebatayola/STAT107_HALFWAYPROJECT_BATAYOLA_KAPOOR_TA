...
#Runs the cleaning script to prepare the dataset for analysis
source("Cleaning.Rmd")
...
This analysis investigates which game statistics serve as the most significant predictors of winning an NBA game. Using team-level game data from the 2022-2023 NBA season, we apply multivariable logistic regression to model the binary outcome of game wins (home team wins = 1, away team wins = 0) and linear regression to predict point differentials. Our models incorporate differential statistics between home and away teams across five key performance metrics: field goal percentage (FG_diff), free throw percentage (FT_diff), three-point percentage (FG3_diff), assists (AST_diff), and rebounds (REB_diff). Preliminary analysis suggests that these differential statistics provide meaningful predictive power for game outcomes. The logistic regression model enables prediction of win probabilities, while the linear regression model estimates expected point margins. This analysis benefits sports analysts, team management, and researchers interested in understanding the statistical factors that contribute to NBA game success. The models demonstrate that team performance differentials, particularly in shooting efficiency and ball movement, are critical indicators of game outcomes.
...
```{r data-loading}
# Load the data (assuming games_clean is created from Cleaning.Rmd)
# If needed, uncomment and run the cleaning code:
# source("Cleaning.Rmd")
```
